...
for epoch in range(100):
    ...
    for i, (images, captions, lengths) in enumerate(loader):
        ...
        targets = captions.view(captions.numel())
        flatoutputs=torch.cat(torch.unbind(outputs,1))
        loss = criterion(flatoutputs, to_var(targets))
        loss.backward()
...
#               torch.save(decoder.state_dict(),'decoder-%d-%d.pkl' %(epoch+1, i+1))
    ...
    for i, (images, captions, lengths) in enumerate(loader2):
        ...
        targets = captions.view(captions.numel())
        flatoutputs=torch.cat(torch.unbind(outputs,1))
        losstest = criterion(flatoutputs, to_var(targets))
        test_loss+=losstest.data[0]
    print(test_loss)
